# 已完成工作单：Q10 G1 第二切片——Course 写侧（Create + PATCH 三态 + version 冲突 + 结构化字段错误）

- 状态：`DONE`（项目负责人 2026-09-23 回复“直接验收并让我收口归档”，即验收通过并授权收口归档与本地提交）
- 归档：2026-09-23 从 `docs/roadmap/ACTIVE_WORK.md` 移入 `docs/roadmap/completed/`
- 验收记录：负责人对执行结果回复“直接验收并让我收口归档”。验收依据：
  1. 五条新语法（`versioned`、`error … status`、`persist … else`、`patch of`、`.present`）贯穿 Parser → Semantic → Lowering → Generator，每条规则落在唯一阶段并带精确码/数量/span 的反例；
  2. 生成工程在真实 MySQL 8.4.11 + HTTP 上完成 16 组请求、**61 项业务断言 0 失败**，含并发同版本竞态“恰好一成功一 409、版本只 +1”与每次失败请求后的整行指纹核对；
  3. Q9 场景在同一棵树上回归 **40/40**（错误契约变更后 Q9 的 400 断言仍成立）；
  4. 全量两个闸门 **BUILD SUCCESS**，合计 **702 run / 0 fail / 0 error / 5 skip**（620 → 702，差量 82 逐模块归因于本单），无新增 skip/exclude；
  5. 八项实施期订正（C1–C8）均已记录证据，其中 C1–C3 为真实运行发现的生成代码缺陷。
- 验收裁决记录（2026-09-23，负责人直接验收，未要求在验收中返工）：
  - C1–C8 全部接受（含 C7“支撑文件必须是无条件 target 产物”的结构约束与 C6 参考环境脚本修复）；
  - 错误信封的码名**保留现状**（信封级 `INVALID_REQUEST` + Bean Validation 字段码），与设计 02 的 `REQUEST_INVALID`/`UNKNOWN_FIELD` 逐字对齐登记为**后续独立项**，不阻断本单；
  - C8 的 `SpringBootModelLowerer.java` 重写**不在本单范围**，登记为独立清理项（本单只保留最小修复与行为等价证据）。
- 所属阶段：**G1：单文件课程业务切片**（第二张工作单；G0 已完成门见 `docs/qualification/CURRENT_QUALIFICATION.md` 第 0 节）
...
- 版本快照：实现与测试由负责人提交为 `92c505c`（消息为 `pause`）；本单验收只产生文档改动，随验收做本地提交
...
| **Q10（本单）** | Course 写侧：Create、Get、PATCH 三态、version 冲突、结构化字段错误 | BIZ-01、BIZ-02、BIZ-03、BIZ-04 | **已完成、验收并归档**（2026-09-23） |
...
**状态：`AWAITING_ACCEPTANCE` → `DONE`（2026-09-21 完成并交接 → 2026-09-23 验收通过并归档）。**
