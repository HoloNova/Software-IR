...
- 状态：**`AWAITING_ACCEPTANCE`**（2026-09-26 按裁决 A 完成身份/拒绝/只读计划；定向测试已过，两条全量闸门待 CI。先决核查 P1–P4 已回填）
- 所属阶段：**G2：持久身份与受控模块**；本单证明单文件改名的身份与计划语义，不宣称改名已安全落盘，更不宣称 G2 或 LANG-01..05 全部门禁完成。
- 前置：G1 已验收归档；`main` 当前基线以 `git log -1` 为准。实现阶段不自动提交或推送。
...
## 实施证据（2026-09-26，本机定向测试；全量闸门待 CI）
...
| 两条全量闸门 + Q9/Q10/Q11/Q13 业务场景 IT | 本机不跑；待获准提交/推送后由 GitHub CI 对待验收 commit 跑**一次**并记 SHA + run URL + artifact |
...
2. 旧源旧身份仍按名称派生：`sourcesWithoutDeclaredIdsKeepTheirNameDerivedIdentities` 的字面断言与 `ToolchainDeterminismTest` 在当前树上两次生成的清单/字节一致；生成器未改。**尚无与旧 commit 的逐字节交叉比对**，不把当前树重复生成一致冒称为跨版本等字节证据。
...
- 本单置 `AWAITING_ACCEPTANCE`：定向证据已齐，全量闸门与业务场景 IT 需 CI 证据后才可判 DONE（本机不跑，不以局部代替）。
...
**交接边界：`AWAITING_ACCEPTANCE`（BLOCK-1 已按 A 裁决，定向证据齐；CI 闸门与业务 IT 未运行）。** 本单只完成身份、拒绝与只读计划；新事务的详细设计与执行证据在下一张单冻结，不把 V4 方案当作本单已实现能力。
