...
> 更新：G0/G1 已验收归档，G1 基线为 GitHub CI 775/0/0/5（查询 40、写侧 61、关联 36、变更闭环 63 项业务断言均通过）；当前 `ACTIVE_WORK.md` 为 **Q16 `AWAITING_ACCEPTANCE`**（G2 单文件持久身份与只读单声明改名计划：定向 change 69、真实管道 9、旧源 12 均 0 失败；原课程全局替换连带 Input 改名会被 `PATH-004` 明确拒绝；全量闸门与业务场景 IT 待 CI；负责人裁决 A：混合事务另立后续单）。
> 读者：项目负责人和后续执行 Agent
...
| 工作单 | 范围 | 状态 |
| --- | --- | --- |
| Q16 | 单文件持久身份、旧身份不明时拒绝改名、独立只读**单声明**改名计划；全工程受管文件差异未覆盖时拒绝；不执行文件应用或数据库迁移 | **`AWAITING_ACCEPTANCE`**：P1–P4 已核查，负责人选 A，真实管道证据与单声明边界已具备（待 CI 全量闸门），见 [`ACTIVE_WORK.md`](ACTIVE_WORK.md) |
| Q17 候选（无开工授权） | **单声明**混合路径变更的一次原子事务、CURRENT/Journal 恢复、旧受管文件清理、应用盘面与候选从零生成逐字节一致；日志 V4 只是待核方案 | 待 Q16 计划证据与单独工作单审定后立项；Q16 不抵扣本门 |
