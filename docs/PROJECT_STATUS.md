...
> 更新：G1 完成证据截至 2026-09-23（**G0 完成门已关闭**：Q1–Q8 全部完成并归档；**G1 的 Q9、Q10、Q11、Q13 及其前置修复单 Q14/Q15 全部完成、验收并归档**——全量 **775/0/0/5**（冻结与完成两种形式均 BUILD SUCCESS，由 GitHub CI run `35854153833` 一次跑完），真实 MySQL/HTTP 四个业务场景：查询 40 项、写侧 61 项、关联 36 项、变更闭环 63 项断言全部 0 失败；`ACTIVE_WORK.md` 当前为 Q16 `AWAITING_ACCEPTANCE`（G2 单文件持久身份、旧身份拒绝语义与只读类型化改名计划已实施，定向测试 parser 84 / semantic 184 / lowering 86 / change 69 / 真实管道 9 / 旧源回归 12，0 失败；原课程全局替换同时改名 Input，因 DTO 不在单声明计划里报 `SIR-RENAME-PATH-004` 拒绝；全量闸门待 CI；P3 实测无混合事务路径，负责人裁决 A 将单声明原子应用与盘面一致性移至 Q17 候选，组合改名另待立项），**G1 完成门已满足**，G2 尚未完成；第 3、5 节能力结论沿用日期化运行记录）
